...
export const socialComposerApi = {
  ...
  generateSeries: (payload: { title: string; prompt: string; count: number; interval_days: number; scheduled_for: string; networks: DraftPayload['networks']; controls: DraftPayload['controls'] }) => request<{ posts: SocialPost[] }>('/posts/series/', { method: 'POST', body: JSON.stringify(payload) }),

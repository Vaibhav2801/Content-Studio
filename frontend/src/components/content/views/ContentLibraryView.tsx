import { Archive, BookOpen, Calendar, Copy, ExternalLink, FileText, LoaderCircle, Recycle, Search, Send } from 'lucide-react'
...
export function ContentLibraryView() {
  ...
  const publishNow = async (variant: LibraryPost['variants'][number]) => {
    setBusy(variant.id + '-PUBLISH'); setError(''); setNotice('')
    try {
      if (isDemo) {
        setPosts((current) => current?.map((post) => ({
          ...post,
          variants: post.variants.map((row) => row.id === variant.id ? { ...row, status: 'SUBMITTED' } : row),
        })) ?? current)
        setNotice('Post submitted for publishing. Its status will update automatically.')
      } else {
        const result = await contentStudioApi.publishNow(variant.id)
  ...
  return <section className="studio-screen" aria-label="Content Library">
    ...
    {!posts ? <div className="li-loading" role="status">Loading Content Library…</div> : posts.length === 0 ? <div className="card"><EmptyState icon={BookOpen} title="No content matches" detail="Try clearing a filter or create a new post." action={{ label: 'Create a post', to: '/content/create' }} /></div> : <div className="library-post-list">{posts.map((post) => <article className="card library-post-card" key={post.id}>
      <header>
        <span className="library-file-icon"><FileText size={18} /></span>
        <div>
          <span>{post.source?.label || 'New idea'}</span>
          <h3 style={{ cursor: 'pointer' }} onClick={() => navigate(`/content/create?draft=${post.id}`)}>{post.idea_title}</h3>
          <small><Calendar size={13} /> Updated {new Date(post.updated_at).toLocaleDateString()}</small>
        </div>
        {post.generation_status === 'GENERATING' ? (
          <span className="li-status status-generating" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
            <LoaderCircle className="spin" size={13} /> Generating…
          </span>
        ) : post.generation_status === 'FAILED' ? (
          <span className="li-status status-failed">Generation failed</span>
        ) : (
          <span className={`li-status status-${post.state.toLowerCase()}`}>{post.state.replaceAll('_', ' ')}</span>
        )}
      </header>
      <p>{post.idea_text || post.variants[0]?.copy || 'No summary yet.'}</p>
      <div className="library-platforms">{post.variants.map((variant) => <span key={variant.id}>{variant.network_label} · {variant.status.replaceAll('_', ' ').toLowerCase()}{variant.media.length ? ` · ${variant.media.length} media` : ''}</span>)}</div>
      ...
      <footer>
        <button className="button button-dark" type="button" onClick={() => navigate(`/content/create?draft=${post.id}`)}>
          {post.generation_status === 'GENERATING' ? 'View progress' : post.state === 'DRAFT' ? 'Open in Composer' : 'Review Draft'}
        </button>
        {post.variants.filter((variant) => variant.status === 'APPROVED' || variant.status === 'SCHEDULED').map((variant) => <button key={variant.id} className="button button-dark" type="button" disabled={Boolean(busy)} aria-busy={busy === variant.id + '-PUBLISH'} aria-label={'Publish ' + post.idea_title + ' to ' + variant.network_label + ' now'} onClick={() => void publishNow(variant)}>{busy === variant.id + '-PUBLISH' ? <LoaderCircle className="spin" size={15} /> : <Send size={15} />} Publish now · {variant.network_label}</button>)}
        <button className="li-quiet-button" onClick={() => setDetails(details === post.id ? '' : post.id)}><ExternalLink size={15} /> {details === post.id ? 'Close details' : 'Open details'}</button>
        <button className="li-quiet-button" disabled={Boolean(busy)} aria-busy={busy === post.id + '-DUPLICATE'} onClick={() => void action(post, 'DUPLICATE')}>{busy === post.id + '-DUPLICATE' ? <LoaderCircle className="spin" size={15} /> : <Copy size={15} />} Duplicate</button>
        <button className="li-quiet-button" disabled={Boolean(busy)} aria-busy={busy === post.id + '-REUSE_IDEA'} onClick={() => void action(post, 'REUSE_IDEA')}>{busy === post.id + '-REUSE_IDEA' ? <LoaderCircle className="spin" size={15} /> : <Recycle size={15} />} Reuse idea</button>
        <button className="li-text-button danger" disabled={Boolean(busy)} aria-busy={busy === post.id + '-ARCHIVE'} onClick={() => void action(post, 'ARCHIVE')}>{busy === `${post.id}-ARCHIVE` ? <LoaderCircle className="spin" size={15} /> : <Archive size={15} />} Archive</button>
      </footer>
    </article>)}</div>}
    </>}
  </section>

...
export function MediaManager({ variant, busy, onUpload, onRemove, onReorder, onAltText, onRegenerate }: Props) {
  ...
  return <section className="media-manager" aria-labelledby={`media-title-${variant.id}`}>
    <div className="media-manager-head"><div><h3 id={`media-title-${variant.id}`}>Media</h3><p>Add images, a video, or a document supported by {variant.network_label}.</p></div><div><input ref={input} hidden type="file" accept="image/*,video/mp4,application/pdf" onChange={(event) => { const file = event.target.files?.[0]; if (file) onUpload(file); event.target.value = '' }} /><button type="button" disabled={busy} onClick={() => input.current?.click()}><Upload size={15} /> Upload</button><button type="button" disabled={busy} onClick={() => onRegenerate()}>{busy ? <LoaderCircle className="spin" size={15} /> : <ImagePlus size={15} />} Create image</button></div></div>

import { ArrowRight, CalendarDays, Check, ChevronRight, Instagram, Layers3, Linkedin, ShieldCheck, Sparkles, WandSparkles } from 'lucide-react'
import { Link } from 'react-router-dom'
import { useAuth } from '../components/content/AuthContext'
import './LandingPage.css'

const plans = [
  { name: 'Starter', price: 24, credits: 100, accounts: 1, description: 'For one brand finding its rhythm.', examples: 'About 10 image posts', featured: false },
...
export function LandingPage() {
  ...
  return <div className="landing-page">
    ...
    <main>
      ...
      <section className="landing-section landing-pricing" id="pricing"><div className="landing-section-head"><span className="landing-kicker">PLANNED PRICING</span><h2>Pay for the creative work you use.</h2><p>A monthly plan covers connected accounts and gives you credits for AI creation. Publishing and scheduling do not use credits.</p></div><div className="landing-pricing-grid">{plans.map((plan) => <article className={plan.featured ? 'featured' : ''} key={plan.name}>{plan.featured && <span className="landing-plan-badge">POPULAR CHOICE</span>}<div className="landing-plan-name">{plan.name}</div><p>{plan.description}</p><div className="landing-plan-price"><strong>${plan.price}</strong><span>/ month<br />plus applicable tax</span></div><div className="landing-plan-divider" /><ul><li><Check size={17} /> {plan.credits} AI credits each month</li><li><Check size={17} /> {plan.accounts} connected {plan.accounts === 1 ? 'account' : 'accounts'}</li><li><Check size={17} /> {plan.examples}</li><li><Check size={17} /> Unlimited scheduling and publishing</li></ul><Link className={plan.featured ? 'landing-plan-cta featured' : 'landing-plan-cta'} to={primaryPath}>{primaryLabel} <ArrowRight size={16} /></Link></article>)}</div><p className="landing-pricing-note">Pricing is a proposal; checkout and credit metering are not active yet. A platform draft uses 2 credits and an AI image uses 8. Extra credits and accounts would be available separately.</p></section>
